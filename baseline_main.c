...
int main() {
  ...
  do_baseline_snapshot(kvs);
  clock_t end_snapshot = clock();
  printf("do_baseline_snapshot 실행 시간: %f 초\n", ((double)(end_snapshot - start_snapshot)) / CLOCKS_PER_SEC);

  print_memory_usage("After snapshot");
